...
Write-Host "  而旧代码不认识 hardDryRun，在旧代码上关 dryRun 会真武装硬档。"
Write-Host "  装好不等于已武装，见 README「装完必须重启 dsh」与「怎么确认它已经武装」。）"
Write-Host ""
Write-Host "小结：复制了 preset 2 个文件 + 技能 1 个 + 插件 5 项（package.json/src/README.md/examples/LICENSE）；挂载行 -> $patchNote；preset 改动必须重启 dsh 才生效；插件行改 config: 热重载，但换过 src\ 里的代码之后同样必须重启（旧代码不认识 hardDryRun，别在旧代码上关 dryRun）。"
