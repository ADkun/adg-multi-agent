...
4. **禁止在 persona 里写 token／读取预算**（"结论控制在 N 字符内""委派 prompt 自带读取预算"之类）。该层纪律已整体撤销。**唯一与成本有关的是调度侧两条编排层派发拓扑规则**（同一实体 + 同一性质的任务合并成一次委派；同一实体的后续任务接给已经读过它的那个专家，`preset/design.md` I13）—— 它们约束"派给谁、派几次"，不是"单个专家能读多少、能写多少"；禁止把这两条改写成预算。
...
| 改调度 persona 的名册或分派规则 | `preset/design.md` | `preset/testing-guide.md`（名册与专家行的双向一致性约束）；改**派发拓扑规则**（I13）再读 `README.md`「多智能体的 token 消耗：已落地与可选手段」 |
