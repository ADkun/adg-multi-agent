...
3. **同步更新文件顶部 `persona` 的 prefix**：把新专家加进「可委派的专家」名册，并按需补一条
   ...
   编排层是规则 6 / 7 / 13 / 14（同一实体 + 同一性质的任务合并成一次委派；同一实体的后续任务用
   `list_agents` + `send_message` 接给已经读过它的那个专家；大范围改动先定位再动手；跨专家传递大
   材料走 digest），闸门是规则 11 / 12（浏览器权限前置 + 人工介入转达）；新增专家时只按它的性质
   补一句"该派给谁"，别把名册改写成预算、也别删掉或放宽这几条。
...
- **新专家的 persona 必须继承「收敛纪律」那一行。** 每个专家 persona 的最后一行都是同一句话：
  ...
  净效果是更差的结论。**唯一的例外是调度侧那四条编排层规则**（同实体合并、优先恢复既有专家、
  先定位再动手、digest 中转）：它们管的是"派给谁、派几次、材料怎么中转"，不管"单个专家能读多少、
  写多少"，所以**不要拿这条红线当理由删掉它们**。省 token 交给插件的**步数收敛检查点**（`dsh-adg-token-budget`；它**不比较
