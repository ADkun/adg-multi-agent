...
成本控制分两层：**编排层**是调度 persona 的四条规则（同一实体 + 同一性质的任务只派一次、同一实体
的后续任务接给已经读过它的那个专家、大范围改动先定位再动手、跨专家传递大材料走 digest，见
...
3. 需要多个专家时（**实体不同或性质不同**才拆），它在同一条回复里并行启动多个委派，不串行等待；
   ...
   交付前删掉）—— 见 [多智能体的 token 消耗](#多智能体的-token-消耗已落地与可选手段)。
...
调度者 persona 里现在还多四条**与成本有关**的规则（规则 6 / 7 / 13 / 14：同一实体 + 同一性质的任务
只派一次、同一实体的后续任务接给已经读过它的那个专家、大范围改动先定位再动手、跨专家传递大材料走
digest）。它们与上面被撤销的那一层**不是一回事**，边界写死在 `preset/design.md` 的 I10 / I13 / I14：
被撤销的是**子代理预算**（"你能读多少、结论写多长"，改的是单个专家的行为，代价是漏项与返工）；
留下的是**编排层规则**（"派给谁、派几次、材料怎么中转"，改的是调度者的计划，省掉的是同一份材料被
重复买 N 次，顺带让多路结论不再互相矛盾）。前者已被实测判定为净负，后者不限制任何单个专家的读取量
与产出量。见 [多智能体的 token 消耗](#多智能体的-token-消耗已落地与可选手段)。
...
| 手段 | 状态 | 理由 | 省在哪 / 怎么量 |
|---|---|---|---|
| 同一实体 + 同一性质的任务合并成一次委派（调度 persona 规则 6） | **已落地** | 每个子代理都会把同一份材料从头读一遍；N 个同类委派 = 同一份材料买 N 次 | 直接观测量是**子代理个数**（同一实体的同类委派应只产生 1 个）。规则 6 / 7 合计给调度者 persona 加了 594 字符（prefix 块 4050 字符的 14.7%，按 ~4 字符/token 估算约 150 token/步）——用一次合并换掉一整个子代理的固定开销加全量重读 |
...
| 压缩调度 persona 里与专家重复的浏览器细则（规则 11/12） | **已落地** | 规则 11+12 原先合计 **1281 字符 = prefix 块的 35.6%**，而失败签名、端口 / profile / CDP 重连这些机制细节在 `agent_browser` 的 persona 里已各有一份（2030 字符）；调度者需要的只是**决策 + 转达** | 现已压到 **812 字符 = prefix 块的 20.0%**（−36.6%，约 −117 token/步，按 ~4 字符/token）。压缩**只动措辞不动语义**：`danger-full-access` 判定、`ask_user_question` 三选项、四分支处置、「同一条路径至多一轮」全部保留（`preset/design.md` I11/I12）；失败签名与根因现在只在专家 persona 与 [浏览器专家需要完全权限](#浏览器专家需要完全权限) 里出现 |
...
**这一轮改动的净体积**：规则 13 + 14 给 prefix 块加了 868 字符（313 + 555），同时规则 11 / 12 减掉
469 字符 —— 净 +399 字符（3598 → 4050）。压缩是**为语义腾位置**：不先把与专家重复的机制复述删掉，
再加两条规则就会把调度者的每步系统提示推到 4.4k 字符。也正因为 persona 每加一条都在加固定成本，
新规则只写**判据与动作**，机制细节一律指向专家 persona 与本文。

**怎么判断"合并"真的落地**（不许只看总 token —— 语料是活的，`preset/design.md` 的 I3b 禁止拿基线
直接比）：用下面「怎么重新测量」的审计脚本，看两件事 —— ① `adg` 行的 `requests` 与每个子代理的
`toolChars`；② **同一实体的任务产生了几个子代理**（这才是规则 6 的直接观测量）。
**未观测**：到本次改动为止，还没有一次真实 Adg 会话带着这四条规则跑过，所以"调度者是否真的按
规则 6 合并、按规则 7 恢复、按规则 13 先定位再改、按规则 14 用 digest 并清掉工件"全部属于
**未观测**（量法：在新会话里让同一个代码库做两轮探索，数子代理条数与 `list_agents` /
`send_message` 的调用次数；digest 那条另看交付后工作区 `git status` 是否干净）。
...
- 从 DSH 出厂 preset `standard`（标准模式）复制而来，实质改动是八处：
  ...
  任务结束即删）；同时把浏览器权限闸门与人工介入两条（规则 11 / 12）**压缩措辞**
  （1281 → 812 字符，判定与分支语义未变）—— 这四条规则与压缩的量化见
