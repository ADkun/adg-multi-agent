...
成本控制分两层：**编排层**是调度 persona 的两条派发拓扑规则（同一实体 + 同一性质的任务只派一次、
同一实体的后续任务接给已经读过它的那个专家，见
...
3. 需要多个专家时（**实体不同或性质不同**才拆），它在同一条回复里并行启动多个委派，不串行等待；
   ...
   （`list_agents` + `send_message`），不重新开一个 —— 见
...
### persona 层保留的政策：专家侧的收敛纪律与调度侧的派发拓扑
...
调度者 persona 里现在还多两条**与成本有关**的规则（规则 6 / 规则 7：同一实体 + 同一性质的任务只派
一次、同一实体的后续任务接给已经读过它的那个专家）。它们与上面被撤销的那一层**不是一回事**，
边界写死在 `preset/design.md` 的 I10 / I13：被撤销的是**子代理预算**（"你能读多少、结论写多长"，
改的是单个专家的行为，代价是漏项与返工）；留下的是**编排层拓扑**（"派给谁、派几次"，改的是调度者
的计划，省掉的是同一份材料被重复买 N 次，顺带让多路结论不再互相矛盾）。前者已被实测判定为净负，
后者不限制任何单个专家的读取量与产出量。见
[多智能体的 token 消耗](#多智能体的-token-消耗已落地与可选手段)。
...
| 手段 | 状态 | 理由 | 省在哪 / 怎么量 |
|---|---|---|---|
| 同一实体 + 同一性质的任务合并成一次委派（调度 persona 规则 6） | **已落地** | 每个子代理都会把同一份材料从头读一遍；N 个同类委派 = 同一份材料买 N 次 | 直接观测量是**子代理个数**（同一实体的同类委派应只产生 1 个）。规则 6 给调度者 persona 加了 594 字符（prefix 块 3598 字符的 16.5%，按 ~4 字符/token 估算约 150 token/步）——用一次合并换掉一整个子代理的固定开销加全量重读 |
...
| 压缩调度 persona 里与专家重复的浏览器细则（规则 11/12） | **建议，需权衡** | 规则 11+12 合计 **1281 字符 = prefix 块的 35.6%**，而失败签名、端口 / profile / CDP 重连这些机制细节在 `agent_browser` 的 persona 里已各有一份（2030 字符）；调度者需要的只是**决策 + 转达** | 调度者每一步的系统提示都会短这一截（调度者占账单 59%）。**代价**：闸门语义不能丢（`preset/design.md` I11/I12），改完必须人工复核 `preset/testing-guide.md` 的 I11 L1 与 I12 M2 两条用例 |
| 探索型委派回传"可复用工件"（digest 文件），后续专家读工件而不是重读源材料 | **建议，未落地** | 同一批材料要被 3 个以上专家看时，第二次之后的读取可以换成读工件 | 需要约定工件路径、过期判断与清理；只对"多专家 + 大材料 + 材料稳定"划算，否则工件本身变成新的噪音 |
| 大仓库改动先让 `agent_researcher` 产出 `path:line`，再让 `agent_coder` 按位改 | **建议（性质不同的正例）** | 两者性质不同（只读调研 vs 写入改动），**本来就该拆**；researcher 的输出正好是 coder 的输入 | 多一跳，但 coder 的步数与读取量下降。这是规则 6 的**反例校准**：拆要看维度，不是一律不拆 |
...
**未观测**：到本次改动为止，还没有一次真实 Adg 会话带着这两条新规则跑过，所以"调度者是否真的按
规则 6 合并、按规则 7 恢复"属于**未观测**（量法：在新会话里让同一个代码库做两轮探索，数子代理
条数与 `list_agents` / `send_message` 的调用次数）。
...
[persona 层保留的政策](#persona-层保留的政策专家侧的收敛纪律与调度侧的派发拓扑)。提醒以调度者的名义、
按下面的阶梯注入：
...
- 从 DSH 出厂 preset `standard`（标准模式）复制而来，实质改动是七处：
  ...
  已经读过它的那个专家，见 [多智能体的 token 消耗](#多智能体的-token-消耗已落地与可选手段)）。
