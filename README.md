...
> 包已部署到真机路径并与仓库逐字节一致；重启后的激活行出现了 `stepNudge=true stepTiers=[…]`
> 与 `hardDryRun=true` 三个新字段，说明**加载与解析都已实测**（为什么必须重启见
> [装完必须重启 dsh](#装完必须重启-dsh)：热重载只重放 config，不会重新 import 已加载的模块）。
> 但**"那条中文提醒真的进了子代理的转写、且它因此收敛了"仍然一次都没观测过**：
> 重启之后还没有受管子代理走到第一个 tier，所以日志里 `step stage:` 那两类行都还是 0 行。
>
> **同一天晚些时候，阶梯从 `[12, 24, 40]` 换成 `[4, 8, …, 280]`、措辞从三段递进的命令式
> 换成一条"可选、二选一"的提醒。** 依据是重跑审计后的真实分布（中位数 39、p10 只有 6：
> 按均值放检查点等于不问），而不是"多数子代理不需要那么多步"这个直觉本身 ——
...
> 这是**代码改动**，需要再重启一次才生效；`stepTiers` 那一半在那之后按 `config:` 改（热重载）。
>
...
- **真实触发行为**：`hard stage: cancel` 已经在真机观测到（上面第 3 行）；`dryRun: true` 期间
  ...
  都还没有**——它们在 2026-09-25 01:38:47 起已武装，但还没有受管子代理越过 210 万或走到第一个
  tier。见 [现在的证据到哪为止](#现在的证据到哪为止)。

**当前这一行是武装状态**（`enabled: true`、`dryRun: false`、`hardDryRun: true`），加载那一半
已经实测：重启后的激活行里出现了 `stepNudge=true stepTiers=[…] … hardDryRun=true`，说明
**新代码真的在跑、`stepTiers` 解析成的是阶梯而不是回落默认**。所以现在只要真机日志里出现
`step stage: nudged …`，那条提醒就确实被注入了一次 —— 那将是这个功能的第一条端到端证据，
目前还没有。

**阶梯与措辞在 2026-09-25 当天换过一次**（`[12, 24, 40]` + 三段递进命令式 →
`[4, 8, …, 280]` + 一条可选的二选一提醒）。这是代码改动，**要再重启一次**才生效；
`stepTiers` 那一半在那之后按 `config:` 改（热重载，不用重启）。换之前必须先重启的理由在
[装完必须重启 dsh](#装完必须重启-dsh)：**当前内存里还是旧模块，而旧模块会把 14 档阶梯里的
第 3 档之后全部映射到它最硬的那段正文** —— 先改 `stepTiers` 再重启，会得到一个"每一步都用
最硬措辞提醒"的窗口。
...
| **步数检查点**（`step stage: nudged`）在真机上发生 | **未观测**（**但加载与武装已实测**）：重启后的激活行带三个新字段，证明新代码在跑、`stepTiers` 解析成的是阶梯而不是回落；只是重启后还没有受管子代理走到第一个 tier，所以日志里 `step stage:` 的两类行都是 0 行 |
| **更早更密的阶梯 + 选择式措辞到底有没有用** | **未观测，而且是这个功能的核心问题**：分布（37 个子代理，中位数 39、p10 6）与成本（214 条约占 0.25%）是实测的；"收到 9 条检查点的子代理是否比不收到时更早收敛"没有任何证据。量法写在插件 README 里：`audit-steps.mjs` 前后各跑一次，比分位数 |
