...
test('apply always writes exactly one activation line, enabled or not', (t) => {
  ...
  assert.match(
    enabledLines[0],
    /budgetTokens=3000000 softThreshold=2100000 softRatio=0\.7 presets=\[adg\] cacheReadWeight=1 softNudge=true dryRun=false logFile=/,
  )
...
test('activationLine reports the resolved configuration on one line', () => {
  const line = activationLine(
    normalizeConfig({ enabled: true, budgetTokens: 1000, softRatio: 0.5, presets: ['adg'], dryRun: true, logFile: 'D:\\x.log' }),
    'module-fallback:web',
  )
  ...
  assert.match(line, /dryRun=true logFile='D:\\x\.log'$/)
...
test('a governed child below the soft threshold passes through', async () => {
  ...
  assert.equal(next.calls, 1)
  assert.equal(agent.calls.cancel.length, 0)
  assert.equal(handler.testState.sessions.size, 0, 'a passing step must not allocate session state')
})
...
test('dryRun soft injects nothing and logs the nudge it would have made', async (t) => {
  ...
  // A calibration run must not allocate the once-per-session flag either.
...
test('dryRun does not consume the one-shot flag, so arming still nudges', async (t) => {
  ...
  assert.equal(dryHandler.testState.sessions.size, 0, 'dryRun must leave the per-session state untouched')
...
test('missing sessionProjections passes through without crashing or cancelling', async () => {
  ...
  assert.equal(next.calls, 1)
  assert.equal(agent.calls.cancel.length, 0)
  // A child with no projection has nothing to nudge or stop, so it must not
  // allocate per-session state either.
  assert.equal(handler.testState.sessions.size, 0)
})
...
test('a child that only passes through never allocates session state', async () => {
