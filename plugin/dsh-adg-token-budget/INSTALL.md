...
- **步数检查点已经真机注入过，而且有子代理那一侧的证据。** 三次注入（都是 `tier=1/3 step=12`，
  17:45:49 / 17:55:04 / 18:22:13），第三次用的是新的**选择式**措辞；每个子代理的
  `session.v3.jsonl.zstd` 里都能找到那条消息本身（`role: user`、
  `source: {kind:'plugin', plugin:'dsh-adg-token-budget'}`），与日志行毫秒级对齐，
  第 12 步时它们只花了 10–14 万 token。
  **还没覆盖**：第一个 tier 之外的档、新阶梯（`[4, 8, …, 280]`）下的注入、与 token 软档同一步的
  折叠路径、以及 `dry-run step stage` 行 —— 这台机器从"没有这个 stage"直接到"已武装"，中间没停过。
  （顺序仍然是硬的：部署代码 → 重启 dsh → 确认激活行出现 `stepText=` → 才动 `dryRun` / `stepTiers`。
  旧模块会把 14 档里第 3 档之后全部映射到它最硬的那段正文，先改阶梯会得到一个"每一步都用最硬措辞"的窗口。）
...
- **注入的提醒是否真的让子代理收敛得更快**：三个子代理对提醒做出了反应（两个说要收敛、
  ...
  不足以说明它让任何东西变快了。`settled: released session state …` **已经观测到**（17:55:33）；
  **调度者怎么渲染部分输出**、**恢复的子代理会不会被再次提醒**仍未观测。
